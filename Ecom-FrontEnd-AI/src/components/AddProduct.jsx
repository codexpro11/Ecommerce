...
const AddProduct = () => {
  ...
  const generateDescription = async () => {
    if (!product.productName.trim() || !product.category) {
  ...
  const generateImage = async () => {
    if (!product.productName.trim() || !product.category || !product.description.trim()) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        setProduct({
          productName: generatedProduct.productName || "",
  ...
  const canGenerateDescription = product.productName.trim() && product.category;
  const canGenerateImage = product.productName.trim() && product.category && product.description.trim();
          ...
          <div className="card shadow">
            <div className="card-body">
              <div className="d-flex justify-content-between align-items-center mb-4">
                <h2 className="card-title mb-0">Add New Product</h2>
                <button
                  type="button"
              ...
              <form className="row g-3 needs-validation" noValidate validated={validated.toString()} onSubmit={submitHandler}>
                <div className="col-md-6">
                  ...
                  <input
                    type="text"
                    className={`form-control ${validated ? (errors.productName ? 'is-invalid' : 'is-valid') : ''}`}
